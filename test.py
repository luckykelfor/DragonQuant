...
def get_volum(dragonex, check_type, code, mock=True):
    global global_test_volume
    if mock: return global_test_volume
...
def add_customed_order_sell(dragon, symbol_id, price, volume, mock=True):
    global global_test_volume
    if mock:
        global_test_volume -= volume
    else:
        dragonex.add_order_sell(symbol_id, price, volume)

def add_customed_order_buy(dragon, symbol_id, price, volume, mock=True):
    global global_test_volume
    if mock:
        global_test_volume += volume
    else:
        dragonex.add_order_buy(symbol_id, price, volume)

if __name__ == '__main__':
    global_test_volume = 40
    ...
    # for i in r.data:
    #     for key in i:
    #         if key == 'code' and i[key] in ['dt', 'ae', 'eos']:
    #             print(i)
    ...
    initial_price_hold = float(dragonex.get_market_real(symbol_map["eos_usdt"]).data[0]['close_price'])
    ...
    while True:
        print("开始准备第",cnt,"次交易")
        check_cnt = 1
        # if get_volum(dragonex, "available", "eos") > 1.0:
        while (new_check_price - initial_price_hold) / initial_price_hold < 0.01 and (new_check_price - initial_price_hold) / initial_price_hold > -0.01:
            # buy_market = dragonex.get_market_buy(symbol_map["eos_usdt"])
            time.sleep(6)
            new_check_price = float(dragonex.get_market_real(symbol_map["eos_usdt"]).data[0]['close_price'])
            print("第", check_cnt,"次查询价格为:", new_check_price)
            time.sleep(10)
            check_cnt += 1

        print("准备交易")
        available_volum = get_volum(dragonex, 'available', 'eos')
        print("可用货币数量为:", available_volum)
        time.sleep(2)

        if available_volum > 1.0:
            print("挂交易卖单 价格:", new_check_price, " 数量:", available_volum)
            add_customed_order_sell(dragonex, symbol_map['eos_usdt'], new_check_price, available_volum)
            time.sleep(2)
            print("开始检查是否交易成功")
            check_deal_cnt = 1
            while get_volum(dragonex, 'frozen', 'eos') > 0.01:
            ...
            # dragonex.add_order_sell(symbol_map['eos_usdt'], new_check_price, buy_volume)
            add_customed_order_buy(dragonex, symbol_map['eos_usdt'], new_check_price, buy_volume)
            check_deal_cnt = 1
            while get_volum(dragonex, "available", "eos") < 1.0:
                print("第",check_deal_cnt,"次检查交易是否成功")
                time.sleep(2)
                check_deal_cnt += 1
